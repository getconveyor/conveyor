...
const mapMLModelToModel = (mlModel: MLModel): Model => {
  // Get status from latest_version if available
  const versionStage = mlModel.latest_version?.stage || "none";
  const status: ModelStatus =
    versionStage === "production"
      ? "deployed"
      : versionStage === "staging"
      ? "ready"
      : versionStage === "archived"
      ? "failed"
      : "ready";
  ...
  return {
    ...
    framework: mlModel.framework || "unknown",
    version: `v${mlModel.version_count || 1}`,
    status,
    accuracy: "-",
    lastTrained: mlModel.updated_at
      ? new Date(mlModel.updated_at).toLocaleString()
      : "Unknown",
    trainedBy: mlModel.created_by_name || "System",
  };
};
...
export default function ModelsPage() {
  ...
  const handleDownload = (model: Model) => {
    const blob = new Blob(["Model weights placeholder"], {
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this model and all its versions. This
